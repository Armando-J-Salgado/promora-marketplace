...
class GlobalUsageValidator extends PromocodeValidationHandler
{
    public function handle(Order $order, Promocode $promocode): void
    {
        ...
        if ($globalLimit === null) {
            Logger::getInstance()->log("[FAIL] GlobalUsageValidator | code=usage_limit_reached | promocode=#{$promocode->id} | order=#{$order->id} | El máximo global no ha sido definido para este cupón");
        ...
        if ($redemptionsCount >= $globalLimit) {
            Logger::getInstance()->log("[FAIL] GlobalUsageValidator | code=usage_limit_reached | promocode=#{$promocode->id} | order=#{$order->id} | El código promocional ya ha superado el número máximo de canjes globales");
            throw new InvalidArgumentException('El código promocional ya ha superado el número máximo de canjes globales');
        }

        Logger::getInstance()->log("[PASS] GlobalUsageValidator | promocode=#{$promocode->id} | order=#{$order->id} | regla superada");
