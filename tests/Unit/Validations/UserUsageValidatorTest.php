...
it('passes when there is no user usage limit defined', function () {
    ...
    expect(Logger::getInstance()->getLogs())
        ->toContain("[PASS] UserUsageValidator | promocode=#{$promocode->id} | order=#{$order->id} | regla superada");
});
...
it('passes when customer redemptions are below the limit', function () {
    ...
    expect(Logger::getInstance()->getLogs())
        ->toContain("[PASS] UserUsageValidator | promocode=#{$promocode->id} | order=#{$currentOrder->id} | regla superada");
});
...
it("throws exception when 'El usuario ha excedido el número máximo de usos permitidos para este código promocional'", function () {
    ...
    expect(Logger::getInstance()->getLogs())
        ->toContain("[FAIL] UserUsageValidator | code=usage_limit_reached | promocode=#{$promocode->id} | order=#{$currentOrder->id} | El usuario ha excedido el número máximo de usos permitidos para este código promocional");
});
